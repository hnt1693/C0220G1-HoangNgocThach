...
function users_List() {
    ...
    users.forEach((user) => {
            tableHtml += "<tr>" + "<td>" + id + "</td>" +
                ...
                "<td >" + user["sex"] + "</td>" +
                "<td>" + user["birthday"] + "</td>" +
                "<td><button class = \"btn btn-info" + "\"  id=\"edit" + id + "\" onclick='editUser(" + (id - 1) + ")'>Edit</button>  <button class = \"btn btn-danger" + "\"  id=\"delete" + id + "\" onclick='delete_User(" + (id - 1) + ")'>Delete</button>  <button class = \"btn btn-success" + "\"  id=\"delete" + id + "\" onclick='delete_User(" + (id - 1) + ")'>Tính tiền</button></td>";
            id = id + 1;
        }
    );
